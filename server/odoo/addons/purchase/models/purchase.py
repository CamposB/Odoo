...
class PurchaseOrder(models.Model):
    ...
    def _amount_all(self):
        for order in self:
            expense_recalculation = order.expense_recalculation
            ...
            order.update({
                ...
                'expense_recalculation': expense_recalculation,
                'amount_total': amount_untaxed + amount_tax + expense_recalculation
            })
            order._amount_recalculation()
    ...
    amount_untaxed = fields.Monetary(string='Untaxed Amount', store=True, readonly=True, digits=(16, 4), compute='_amount_all', tracking=True)
    tax_totals_json = fields.Char(compute='_compute_tax_totals_json', digits=(16, 4), readonly=False)
    amount_tax = fields.Monetary(string='Taxes', store=True, readonly=False, digits=(16, 8), compute='_amount_all')
    amount_total = fields.Monetary(string='Total', store=True, readonly=False, digits=(16, 4), compute='_amount_all')
    ...
    expense_recalculation = fields.Float(string='Diferença', store=True, compute='_amount_all' )
    tax_recalculation = fields.Float(string='Impostos', store=True, digits=(16,4), compute='_amount_recalculation' )

    # @api.depends('order_line.price_total')
    # def _amount_all_expense(self):
    #     for order in self:
    #         # Se o valor de amount_recalculation for maior que zero, some-o a amount_total
    #         if order.expense_recalculation > 0:
    #             order.amount_total += order.expense_recalculation


    def _amount_recalculation(self):
        for record in self:
            if record.amount_tax == 0.0:
                record.tax_recalculation = 0.0
            else:
                record.tax_recalculation = record.amount_total - record.amount_untaxed
