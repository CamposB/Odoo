...
class SaleReport(models.Model):
    ...
    def _get_report_values(self, docids, data=None):
        # Obtenha o objeto Sale Order (ou outro objeto relevante) com base nos docids
        sale_orders = self.env['sale.order'].browse(docids)

        # Filtra as linhas do pedido para remover as linhas de entrega
        order_lines = []
        for order in sale_orders:
            order_lines += order.order_line.filtered(lambda x: not x.is_delivery)

        # Formate os valores para duas casas decimais
        for line in order_lines:
            line.price_subtotal = round(line.price_subtotal, 2)
            # Repita para outros campos, se necessário

        # Agora, você pode passar as order_lines filtradas para o relatório PDF
