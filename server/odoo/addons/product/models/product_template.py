...
class ProductTemplate(models.Model):
    ...
    # # Barcode genarated from Default_code  
    # barcode_custom = fields.Char(
    #     'BarcodeCustom', copy=False,
    #     help="International Article Number used for product identification.")
    
    # barcode_custom_image = fields.Binary(
    #     'BarcodeCustom Image', compute='_compute_barcode_custom_image',
    #     help="Barcode image generated from BarcodeCustom.")

    # def generate_barcode(self, input_value):
    #     """
    #     Generate a barcode using the provided input_value and save it as an image.
    #     """
    #     if input_value:
    #         # Choose a barcode symbology (e.g., 'code128')
    #         barcode_type = 'code128'

    #         # Generate the barcode using the provided input_value
    #         code = barcode.get(barcode_type, input_value, writer=ImageWriter())

    #         # Save the barcode as an image file and get the image data
    #         with open('../static/img/barcode.png', 'wb') as f:
    #             code.write(f)
    #             f.seek(0)
    #             barcode_data = base64.b64encode(f.read())

    #         return barcode_data

    # @api.depends('default_code')
    # def _compute_barcode_custom_image(self):
    #     """
    #     Compute the barcode_custom_image field using the default_code.
    #     """
    #     for template in self:
    #         if template.default_code:
    #             # Call the generate_barcode method with default_code as input
    #             barcode_data = template.generate_barcode(template.default_code)
    #             template.barcode_custom_image = barcode_data
    #         else:
    #             template.barcode_custom_image = False
